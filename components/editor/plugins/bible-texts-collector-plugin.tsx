  ...
  useEffect(() => {
    ...
    const collectLinks = (): { key: string; text: string }[] => {
      ...
      const visit = (node: LexicalNode) => {
        ...
        const type = typeof anyNode.getType === "function" ? anyNode.getType() : null
    ...
    const fetchContentForInfo = async (info: { book: string; chapter: number; verse?: number; verses?: number[] }): Promise<{ title: string; content: string }> => {
    ...
    const performCollection = async (links: { key: string; text: string }[]) => {
      try {
        ...
        for (const { key, text } of links) {
          ...
          console.log("[BibleTextsCollector] referências extraídas de", plain, infos)
